...
module.exports.minify = (file_path, user_options) => {
	// Options
	if(user_options){
		if(user_options.excludes) options.excludes = user_options.excludes;
		if(user_options.minify && typeof user_options.minify.replace_variables !== 'undefined') options.minify.replace_variables = user_options.minify.replace_variables;
		if(user_options.minify && typeof user_options.minify.remove_whitespace !== 'undefined') options.minify.remove_whitespace = user_options.minify.remove_whitespace;
		if(user_options.minify && typeof user_options.minify.remove_comments !== 'undefined') options.minify.remove_comments = user_options.minify.remove_comments;
		if(user_options.output) options.output = user_options.output;
	}

	return new Promise((resolve, reject) => {
		// Reads the file
		fs.readFile(file_path, 'utf8', (err, file_data) => {
			if (err) reject('Error: Can’t Read From the Source File or Disk');

			// Check if its a .php file
			if(path.extname(file_path) != ".php")
			...
			let tokens = parser.tokenGetAll(file_data);
